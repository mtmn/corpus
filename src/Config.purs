...
loadConfig path = do
  json <- makeAff \cb ->
    loadConfigImpl path
      ...
      (\j -> cb (Right j))
      *> pure nonCanceler
  baseConfig <- case decodeAppConfig json of
  ...
  let fullConfig = baseConfig { users = map (\u -> u { config = fillCreds u.config }) baseConfig.users }
...
decodeAppConfig :: Json -> Either String AppConfig
decodeAppConfig json = do
  obj <- mapLeft show $ decodeJson json
  port <- mapLeft show $ obj .: "port"
  usersArr <- mapLeft show (obj .: "users" :: Either _ (Array Json))
  users <- traverse decodeUserEntry usersArr
  pure { port, users }
